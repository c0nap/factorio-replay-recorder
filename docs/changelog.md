...
### 0.1.2 — Development Efficiency

Unreleased. Regenerating a replay after a mod change should take a
command, not a person:

- A recorded replay becomes reusable across mod versions: follow the
  testing checklist once, with recording enabled, to capture an "ideal
  replay" exercising every event type this mod produces. From then on,
  headless Factorio's `--run-replay` flag regenerates `replay.json` from
  that same recording on its own, with nobody at the keyboard.
- A companion unlock tool bridges the gap a reused replay would otherwise
  hit against a newer mod build: it keeps whatever settings a save
  already has, fills in defaults for any settings introduced since the
  replay was recorded, and bumps the save's recorded mod version so a
  newer build accepts feeding it a replay recorded on an older one.
- A packaging tool in `tools/` builds an installable mod zip directly,
  replacing the manual steps in [`packaging`](packaging.md) - one command
  instead of a copy-paste release process, useful for cutting the
  frequent test builds this workflow now needs as much as for an actual
  release.

## v0.2 — Development Process

Unreleased. Not about new tracked data - about making sure development
can keep moving without friction and is built on a design that holds up:
...
- Prototype scanning and classification handles a prototype whose
  action/delivery/effect shape doesn't match what this mod expects,
  instead of just logging it as skipped. Factorio's data stage can hand
  back inconsistent shapes for the same conceptual field even within a
  single version - a weapon's `target_effects` entries mixing tables with
  bare strings/booleans/numbers across different prototypes is one
  observed case - and today those get flagged and dropped rather than
  classified.
- Dynamically reading the official [Factorio data repository](https://github.com/wube/factorio-data)
  to check what a new version actually changed, instead of relying on a
  relaunch-only data-stage probe or manually reading a forum changelog
  for every prototype/field this mod depends on.
...
Not every targeted version needs its own adapter, though. The same
unlock tool 0.1.2 uses to replay across a mod version bump can do the
same across a small Factorio version bump, whenever the official
changelog shows nothing that would produce a replay discrepancy for the
recording in question - no changed prototype behavior, no reported
crashes. Bumping from 2.0.76 to 2.0.77, for example, needed nothing more
than that: the [forum release notes](https://forums.factorio.com/viewtopic.php?t=133653)
turned up nothing that would affect a non-Space-Age recording. A real
per-version adapter only becomes necessary once a version actually
changes something this mod's recording depends on.

...
few seconds of waiting or one small manual action (drag an item, click
...
```
/c game.player.cheat_mode=true; game.player.force.research_all_technologies(); game.speed=3; settings.global["rrec-distant-sample-interval-seconds"] = {value = 1}; settings.global["rrec-chain-near-hops"] = {value = 1}; settings.global["rrec-chain-max-hops"] = {value = 50}
```
...
```
/c game.player.insert{name="stone-wall", count=10}; game.player.insert{name="gate", count=2}; game.player.insert{name="gun-turret", count=1}; game.player.insert{name="laser-turret", count=1}; game.player.insert{name="flamethrower-turret", count=1}; game.player.insert{name="firearm-magazine", count=50}
```
**Action:** Place the wall, a gate, and all three turrets near you.
```
/c local ft = game.player.surface.find_entities_filtered{type = "fluid-turret", position = game.player.position, radius = 20}[1]; if ft then ft.insert_fluid{name = "crude-oil", amount = 100} end; local b = game.player.surface.create_entity{name = "behemoth-biter", position = {game.player.position.x + 4, game.player.position.y}, force = "enemy"}; b.health = 1; pcall(function() local w = game.player.surface.create_entity{name = "medium-worm-turret", position = {game.player.position.x + 4, game.player.position.y + 3}, force = "enemy"}; w.health = 1 end); local s = game.player.surface.create_entity{name = "biter-spawner", position = {game.player.position.x + 4, game.player.position.y - 3}, force = "enemy"}; s.health = 1; game.player.surface.create_entity{name = "small-spitter", position = {game.player.position.x + 6, game.player.position.y}, force = "enemy"}
```
...
```
/c game.player.insert{name="car", count=1}; game.player.insert{name="tank", count=1}
```
...
```
/c game.player.character.health = 5; game.player.surface.create_entity{name="small-biter", position={game.player.position.x + 1, game.player.position.y}, force="enemy"}
```
...
ground near you.
```
/c local i = game.player.surface.find_entities_filtered{type = "item-entity", position = game.player.position, radius = 10}[1]; if i then i.destroy() end
```

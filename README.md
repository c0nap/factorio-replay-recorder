...
Documented here rather than left for someone to discover and assume is a bug. Most of the original list got resolved once real API docs came back - see below for what changed and why.

* **A ground item picked up by walking over it doesn't get its cache entry cleaned up.** An item-entity *destroyed* (fire, explosion) fires `on_entity_died` like any other entity and is cleaned up the same way vehicles/containers/robots/inserters are (see `CACHE_CLEANUP_PREFIX` in `script/tracker.lua`) - but there's no confirmed event carrying an entity reference for the "picked up by walking over it" removal path (`on_picked_up_item` only carries `item_stack`/`player_index`, no entity), so that specific removal path can leave a stale `storage.inventory_cache["ground_item_"..id]` entry behind. Same category of gap corpses had before `on_character_corpse_expired` fixed it - if there's a confirmed event for this removal path, it'd close the same way.

Resolved this round, kept here briefly for context on what changed:

* ~~Ground items (`item-entity`) were entirely unimplemented~~ - now tracked the same way corpses are: `LuaEntity.stack` (confirmed valid when `type == "item-entity"`) is diffed every tick via the per-tick physical scan, and `on_player_dropped_item` gives a one-time `ground_item_created` provenance record for manually-dropped items (items reaching the ground some other way - inserter overflow, explosion debris - still get picked up by the per-tick scan, just without that provenance record, since no other confirmed creation-side event exists yet).
